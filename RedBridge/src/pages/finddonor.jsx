...
const FindDonor = () => {
  ...
  useEffect(() => {
    // Fetch donor list from backend
    const fetchDonors = async () => {
      try {
        const res = await fetch("http://localhost:4000/donor/all");
        const data = await res.json();

        const donorsWithAvailability = data.map((donor, idx) => ({
          ...donor,
          id: donor._id || idx + 1, // fallback if no _id
          available: calculateAvailability(donor.lastDonationDate),
        }));
